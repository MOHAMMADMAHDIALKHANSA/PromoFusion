using MarketingHub.Models;
using MarketingHub.Models;
using Microsoft.AspNetCore.Identity;
using Microsoft.AspNetCore.Identity.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore;
using System.Net.NetworkInformation;

namespace MarketingHub.Data
{

    public class ApplicationDbContext : IdentityDbContext
    {
        public ApplicationDbContext(DbContextOptions<ApplicationDbContext> options) : base(options)
        {

        }
        public DbSet<Review> Reviews { get; set; }
        public DbSet<Administrator> Administrators { get; set; }
        public DbSet<MarketingAgency> MarketingAgency { get; set; }
        public DbSet<Location> Locations { get; set; }
        public DbSet<Customer> Customers { get; set; }
        public DbSet<ApplicationUser> applicationUsers { get; set; }
        public DbSet<Post> Posts { get; set; }
        public DbSet<Appointment> Appointments { get; set; }
        public DbSet<Feedback> Feedbacks { get; set; }

        public DbSet<Category> Categories { get; set; }
        public DbSet<ContractType> ContractTypes { get; set; }
        public DbSet<MarketingAgencyContractType> MarketingAgencyContractTypes { get; set; }
        public DbSet<Proposal> Proposals { get; set; }
        public DbSet<MarketingAgencyCategory> MarketingAgencyCategory { get; set; }
        public DbSet<Payment> Payments { get; set; }
        public DbSet<Report> Reports { get; set; }
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {

            base.OnModelCreating(modelBuilder);

            modelBuilder.Entity<Report>()
       .HasOne(r => r.MarketingAgency)
       .WithMany(m => m.Reports)
       .HasForeignKey(r => r.MarketingAgencyId)
       .OnDelete(DeleteBehavior.Cascade);

            modelBuilder.Entity<MarketingAgencyContractType>()
                .HasKey(ma => new { ma.MarketingAgencyId, ma.ContractTypeId });

            modelBuilder.Entity<MarketingAgencyContractType>()
                .HasOne(ma => ma.MarketingAgency)
                .WithMany(m => m.MarketingAgencyContractTypes)
                .HasForeignKey(ma => ma.MarketingAgencyId)
                .OnDelete(DeleteBehavior.Restrict);

            modelBuilder.Entity<MarketingAgencyContractType>()
                .HasOne(ma => ma.ContractType)
                .WithMany(c => c.MarketingAgencyContractTypes)
                .HasForeignKey(ma => ma.ContractTypeId)
                .OnDelete(DeleteBehavior.Restrict);

            modelBuilder.Entity<MarketingAgencyCategory>()
                .HasKey(mc => new { mc.MarketingAgencyId, mc.CategoryId });

            modelBuilder.Entity<MarketingAgencyCategory>()
                .HasOne(mc => mc.MarketingAgency)
                .WithMany(m => m.MarketingAgencyCategories)
                .HasForeignKey(mc => mc.MarketingAgencyId)
                .OnDelete(DeleteBehavior.Restrict);

            modelBuilder.Entity<MarketingAgencyCategory>()
                .HasOne(mc => mc.Category)
                .WithMany(c => c.MarketingAgencyCategories)
                .HasForeignKey(mc => mc.CategoryId)
                .OnDelete(DeleteBehavior.Restrict);


            modelBuilder.Entity<Customer>()
                .HasMany(c => c.Feedbacks)
                .WithOne(f => f.Customer)
                .HasForeignKey(f => f.CustomerId)
                .OnDelete(DeleteBehavior.Restrict);

            modelBuilder.Entity<MarketingAgency>()
                .HasMany(m => m.Appointments)
                .WithOne(a => a.MarketingAgency)
                .HasForeignKey(a => a.MarketingAgencyId);


            modelBuilder.Entity<MarketingAgency>()
    .HasOne(ma => ma.Location)
    .WithMany()
    .HasForeignKey(ma => ma.LocationId)
    .OnDelete(DeleteBehavior.Cascade);



            modelBuilder.Entity<Post>()
             .HasOne(p => p.MarketingAgency)
             .WithMany(c => c.Post)
             .HasForeignKey(p => p.MarketingAgencyId)
             .OnDelete(DeleteBehavior.Cascade);

            modelBuilder.Entity<Feedback>()
               .HasOne(f => f.MarketingAgency)
               .WithMany(m => m.Feedbacks) // Assuming MarketingAgency has a collection of Feedbacks
               .HasForeignKey(f => f.MarketingAgencyId)
               .OnDelete(DeleteBehavior.Restrict);

            modelBuilder.Entity<Feedback>()
           .HasOne(f => f.Customer)
           .WithMany(c => c.Feedbacks)
           .HasForeignKey(f => f.CustomerId);



            modelBuilder.Entity<Proposal>()
               .HasOne(p => p.Customer)
               .WithMany(c => c.Proposals)
               .HasForeignKey(p => p.CustomerId)
               .OnDelete(DeleteBehavior.Restrict);

            modelBuilder.Entity<Proposal>()
                .HasOne(p => p.MarketingAgency)
                .WithMany(m => m.Proposals)
                .HasForeignKey(p => p.MarketingAgencyId)
                .OnDelete(DeleteBehavior.Restrict);

            modelBuilder.Entity<MarketingAgencyCategory>()
            .HasIndex(mac => new { mac.MarketingAgencyId, mac.CategoryId })
            .IsUnique();

            modelBuilder.Entity<MarketingAgencyContractType>()
                .HasIndex(mact => new { mact.MarketingAgencyId, mact.ContractTypeId })
                .IsUnique();
            modelBuilder.Entity<Payment>(entity =>
            {
                entity.Property(e => e.Amount)
                    .HasColumnType("decimal(18,2)");
            });
            modelBuilder.Entity<Report>()
            .HasOne(r => r.MarketingAgency)
            .WithMany(a => a.Reports)
            .HasForeignKey(r => r.MarketingAgencyId)
            .OnDelete(DeleteBehavior.Restrict);

            modelBuilder.Entity<Report>()
                .HasOne(r => r.Customer)
                .WithMany()
                .HasForeignKey(r => r.CustomerId)
                .OnDelete(DeleteBehavior.Restrict);
            // Seed data for ContractType
            modelBuilder.Entity<ContractType>().HasData(
                new ContractType { ContractTypeId = 1, Name = "Full Implementation" },
                new ContractType { ContractTypeId = 2, Name = "Material" },
                new ContractType { ContractTypeId = 3, Name = "Strategy" }
            );

            // Seed data for Category
            modelBuilder.Entity<Category>().HasData(
                new Category { CategoryId = 1, Name = "Digital Media" },
                new Category { CategoryId = 2, Name = "Video Promotion" },
                new Category { CategoryId = 3, Name = "Image" },
                new Category { CategoryId = 4, Name = "Audio" }
            );
            // Seed data for MarketingAgencyCategory
            modelBuilder.Entity<MarketingAgencyCategory>().HasData(
                new MarketingAgencyCategory { MarketingAgencyId = 1, CategoryId = 1 },
                new MarketingAgencyCategory { MarketingAgencyId = 2, CategoryId = 2 },
                new MarketingAgencyCategory { MarketingAgencyId = 3, CategoryId = 3 },
                new MarketingAgencyCategory { MarketingAgencyId = 4, CategoryId = 4 },
                new MarketingAgencyCategory { MarketingAgencyId = 5, CategoryId = 1 },
                new MarketingAgencyCategory { MarketingAgencyId = 6, CategoryId = 1 },
                new MarketingAgencyCategory { MarketingAgencyId = 7, CategoryId = 1 },
                new MarketingAgencyCategory { MarketingAgencyId = 8, CategoryId = 2 },
                new MarketingAgencyCategory { MarketingAgencyId = 9, CategoryId = 3 },
                new MarketingAgencyCategory { MarketingAgencyId = 10, CategoryId = 2 }
            );

            // Seed data for MarketingAgencyContractType
            modelBuilder.Entity<MarketingAgencyContractType>().HasData(
                new MarketingAgencyContractType { MarketingAgencyId = 1, ContractTypeId = 1 },
                new MarketingAgencyContractType { MarketingAgencyId = 2, ContractTypeId = 3 },
                new MarketingAgencyContractType { MarketingAgencyId = 3, ContractTypeId = 2 },
                new MarketingAgencyContractType { MarketingAgencyId = 4, ContractTypeId = 1 },
                new MarketingAgencyContractType { MarketingAgencyId = 5, ContractTypeId = 2 },
                new MarketingAgencyContractType { MarketingAgencyId = 6, ContractTypeId = 3 },
                new MarketingAgencyContractType { MarketingAgencyId = 7, ContractTypeId = 1 },
                new MarketingAgencyContractType { MarketingAgencyId = 8, ContractTypeId = 2 },
                new MarketingAgencyContractType { MarketingAgencyId = 9, ContractTypeId = 3 },
                new MarketingAgencyContractType { MarketingAgencyId = 10, ContractTypeId = 1 }
            );
            modelBuilder.Entity<Location>().HasData(
              new Location
              {
                  LocationId = 5,
                  City = "Beirut",
                  ImageUrl = "https://www.macentreprise.com/img/projects/general-contracting/MHM%20Business%20Center%20-%20Antelias%20and%20Achrafieh%20-%20Adlieh/IMG_5730.jpg",
                  PostalCode = 51,
                  Region = "Jnah",
                  UrlRoute = "https://g.co/kgs/4xKBRqp",
                  BuildingName = "IDS ",
              },
              new Location
              {
                  LocationId = 2,
                  City = "Beirut",
                  ImageUrl = "https://www.yelleb.com/img/lb/j/1543234930-75-nascode.jpg",
                  PostalCode = 51,
                  Region = "Sin El Fil",
                  UrlRoute = "https://maps.app.goo.gl/XdgeL5jJim1GpW997",
                  BuildingName = "NASCOD",
              },
              new Location
              {
                  LocationId = 3,
                  City = "Beirut",
                  ImageUrl = "https://www.samwoh.com.sg/images/Home_Page/5-Latest-Event.jpg",
                  PostalCode = 51,
                  Region = "Horch Tabet",
                  UrlRoute = "https://maps.app.goo.gl/Pcjhs18qevw8UhAc7",
                  BuildingName = "Samwoh",
              },
              new Location
              {
                  LocationId = 4,
                  City = "Beirut",
                  ImageUrl = "https://thingsthatmatter.me/wp-content/uploads/2017/06/office.png",
                  PostalCode = 51,
                  Region = "Horsh Tabet",
                  UrlRoute = "https://maps.app.goo.gl/Q6Vpxm7iRvT2cVuJ7",
                  BuildingName = "Born Creative",
              },
              new Location
              {
                  LocationId = 7,
                  City = "Beirut",
                  ImageUrl = "https://images.unsplash.com/photo-1506748686214-e9df14d4d9d0",
                  PostalCode = 51,
                  Region = "Hamra",
                  UrlRoute = "https://goo.gl/maps/Zk9rT2PvVtF2",
                  BuildingName = "Hamra Tower",
              },
new Location
{
    LocationId = 8,
    City = "Beirut",
    ImageUrl = "https://images.unsplash.com/photo-1574164575883-716f2c63d58b",
    PostalCode = 51,
    Region = "Achrafieh",
    UrlRoute = "https://goo.gl/maps/j6BzY3JgHkR2",
    BuildingName = "ABC Mall",
},
new Location
{
    LocationId = 9,
    City = "Beirut",
    ImageUrl = "https://images.unsplash.com/photo-1564866657312-25ea38246e1c",
    PostalCode = 51,
    Region = "Downtown",
    UrlRoute = "https://goo.gl/maps/X7dK1mF7VbN2",
    BuildingName = "Beirut Souks",
},
new Location
{
    LocationId = 10,
    City = "Beirut",
    ImageUrl = "https://images.unsplash.com/photo-1601600589881-2b5d3c4c2a3e",
    PostalCode = 51,
    Region = "Verdun",
    UrlRoute = "https://goo.gl/maps/H2bY1cP2YyF3",
    BuildingName = "Dunes Center",
},
new Location
{
    LocationId = 11,
    City = "Beirut",
    ImageUrl = "https://images.unsplash.com/photo-1600490377848-c3f2f2b6b6a6",
    PostalCode = 51,
    Region = "Jal El Dib",
    UrlRoute = "https://goo.gl/maps/Jk3L6M8YzM4",
    BuildingName = "Jal El Dib Tower",
},
new Location
{
    LocationId = 12,
    City = "Beirut",
    ImageUrl = "https://images.unsplash.com/photo-1600585154305-17f18f2c5c4a",
    PostalCode = 51,
    Region = "Gemmayze",
    UrlRoute = "https://goo.gl/maps/Fb8Z2hH7bNj2",
    BuildingName = "Gemmayze Plaza",
}


           );

            var user1 = new ApplicationUser
            {
                Id = "1",
                UserName = "beyondfood@gmail.com",
                Name = "foodcouple",
                NormalizedUserName = "BEYONDFOOD@GMAIL.COM",
                Email = "beyondfood@gmail.com",
                NormalizedEmail = "BEYONDFOOD@GMAIL.COM",
                EmailConfirmed = true,
                PasswordHash = new PasswordHasher<ApplicationUser>().HashPassword(null, "Mm_123")
            };
            user1.PasswordHash = new PasswordHasher<ApplicationUser>().HashPassword(user1, "Mm_123");
            var user2 = new ApplicationUser
            {
                Id = "2",
                UserName = "explorernico@gmail.com",
                Name = "explorernico",
                NormalizedUserName = "EXPLORERNICO@GMAIL.COM",
                Email = "explorernico@gmail.com",
                NormalizedEmail = "EXPLORERNICO@GMAIL.COM",
                EmailConfirmed = true,
                PasswordHash = new PasswordHasher<ApplicationUser>().HashPassword(null, "Mm_123")
            };
            user2.PasswordHash = new PasswordHasher<ApplicationUser>().HashPassword(user2, "Mm_123");
            var user3 = new ApplicationUser
            {
                Id = "3",
                UserName = "evolve@gmail.com",
                Name = "evolve",
                NormalizedUserName = "EVOLVE@GMAIL.COM",
                Email = "evolve@gmail.com",
                NormalizedEmail = "EVOLVE@GMAIL.COM",
                EmailConfirmed = true,
            };
            user3.PasswordHash = new PasswordHasher<ApplicationUser>().HashPassword(user3, "Mm_123");
            var user4 = new ApplicationUser
            {
                Id = "4",
                UserName = "foodcouple@gmail.com",
                Name = "foodcouple",
                NormalizedUserName = "FOODCOUPLE@GMAIL.COM",
                Email = "foodcouple@gmail.com",
                NormalizedEmail = "FOODCOUPLE@GMAIL.COM",
                EmailConfirmed = true,


            };
            user4.PasswordHash = new PasswordHasher<ApplicationUser>().HashPassword(user4, "Mm_123");

            var user5 = new ApplicationUser
            {
                Id = "5",
                UserName = "socialmediaqueen@gmail.com",
                Name = "Social Media Queen",
                NormalizedUserName = "SOCIALMEDIAQUEEN@GMAIL.COM",
                Email = "socialmediaqueen@gmail.com",
                NormalizedEmail = "SOCIALMEDIAQUEEN@GMAIL.COM",
                EmailConfirmed = true,
                PasswordHash = new PasswordHasher<ApplicationUser>().HashPassword(null, "Mm_123")
            };
            user5.PasswordHash = new PasswordHasher<ApplicationUser>().HashPassword(user5, "Mm_123");
            var user6 = new ApplicationUser
            {
                Id = "6",
                UserName = "techtrendsetter@gmail.com",
                Name = "Tech Trendsetter",
                NormalizedUserName = "TECHTRENDSETTER@GMAIL.COM",
                Email = "techtrendsetter@gmail.com",
                NormalizedEmail = "TECHTRENDSETTER@GMAIL.COM",
                EmailConfirmed = true,
                PasswordHash = new PasswordHasher<ApplicationUser>().HashPassword(null, "Mm_123")
            };
            user6.PasswordHash = new PasswordHasher<ApplicationUser>().HashPassword(user6, "Mm_123");
            var user7 = new ApplicationUser
            {
                Id = "7",
                UserName = "fitnessfreak@gmail.com",
                Name = "Fitness Freak",
                NormalizedUserName = "FITNESSFREAK@GMAIL.COM",
                Email = "fitnessfreak@gmail.com",
                NormalizedEmail = "FITNESSFREAK@GMAIL.COM",
                EmailConfirmed = true,
                PasswordHash = new PasswordHasher<ApplicationUser>().HashPassword(null, "Mm_123")
            };
            user7.PasswordHash = new PasswordHasher<ApplicationUser>().HashPassword(user7, "Mm_123");
            var user8 = new ApplicationUser
            {
                Id = "8",
                UserName = "traveljunkie@gmail.com",
                Name = "Travel Junkie",
                NormalizedUserName = "TRAVELJUNKIE@GMAIL.COM",
                Email = "traveljunkie@gmail.com",
                NormalizedEmail = "TRAVELJUNKIE@GMAIL.COM",
                EmailConfirmed = true,
                PasswordHash = new PasswordHasher<ApplicationUser>().HashPassword(null, "Mm_123")
            };
            user8.PasswordHash = new PasswordHasher<ApplicationUser>().HashPassword(user8, "Mm_123");
            var user9 = new ApplicationUser
            {
                Id = "9",
                UserName = "styleguru@gmail.com",
                Name = "Style Guru",
                NormalizedUserName = "STYLEGURU@GMAIL.COM",
                Email = "styleguru@gmail.com",
                NormalizedEmail = "STYLEGURU@GMAIL.COM",
                EmailConfirmed = true,
                PasswordHash = new PasswordHasher<ApplicationUser>().HashPassword(null, "Mm_123")
            };
            user9.PasswordHash = new PasswordHasher<ApplicationUser>().HashPassword(user9, "Mm_123");
            var user10 = new ApplicationUser
            {
                Id = "10",
                UserName = "gamingenthusiast@gmail.com",
                Name = "Gaming Enthusiast",
                NormalizedUserName = "GAMINGENTHUSIAST@GMAIL.COM",
                Email = "gamingenthusiast@gmail.com",
                NormalizedEmail = "GAMINGENTHUSIAST@GMAIL.COM",
                EmailConfirmed = true,
                PasswordHash = new PasswordHasher<ApplicationUser>().HashPassword(null, "Mm_123")
            };
            user10.PasswordHash = new PasswordHasher<ApplicationUser>().HashPassword(user10, "Mm_123");

            var user11 = new ApplicationUser
            {
                Id = "11",
                UserName = "MohammadMahdiALkhansa",
                Name = "MohammadMahdi",
                NormalizedUserName = "MOHAMMADMAHDI",
                Email = "mhmd@gmail.com",
                NormalizedEmail = "MHMD@GMAIL.COM",
                EmailConfirmed = true,
                PasswordHash = new PasswordHasher<ApplicationUser>().HashPassword(null, "Mm_123")
            };
            user11.PasswordHash = new PasswordHasher<ApplicationUser>().HashPassword(user11, "Mm_123");

            var administrator = new Administrator
            {
                AdministratorId = 1,
                FirstName = "MohammadMahdi",
                LastName = "ALkhansa",
                Role = "Administrator", // You can set the role name here if needed
                Email = "mhmd@gmail.com",
                UserId = user11.Id,
                ImgUrl = "https://example.com/image.jpg" // Replace with an actual image URL if available
            };
            modelBuilder.Entity<Administrator>().HasData(administrator);


            var roleId = "05b620cf-3017-4b90-b256-80ab0be74c1a";
            var adminRoleId = "7fa4d532-e8fd-4785-9bc1-953754c75268";
            modelBuilder.Entity<IdentityUserRole<string>>().HasData(
                new IdentityUserRole<string> { UserId = "1", RoleId = roleId },
                new IdentityUserRole<string> { UserId = "2", RoleId = roleId },
                new IdentityUserRole<string> { UserId = "3", RoleId = roleId },
                new IdentityUserRole<string> { UserId = "4", RoleId = roleId },
                new IdentityUserRole<string> { UserId = "5", RoleId = roleId },
                new IdentityUserRole<string> { UserId = "6", RoleId = roleId },
                new IdentityUserRole<string> { UserId = "7", RoleId = roleId },
                new IdentityUserRole<string> { UserId = "8", RoleId = roleId },
                new IdentityUserRole<string> { UserId = "9", RoleId = roleId },
                new IdentityUserRole<string> { UserId = "10", RoleId = roleId },
                new IdentityUserRole<string> { UserId = "11", RoleId = adminRoleId }
            );
            modelBuilder.Entity<ApplicationUser>().HasData(user1, user2, user3, user4,user5, user6, user7, user8, user9, user10, user11);

            // Seed MarketingAgencies
            modelBuilder.Entity<MarketingAgency>().HasData(
                new MarketingAgency
                {
                    MarketingAgencyId = 1,
                    Username = "BeyonFood",
                    PhoneNumber = "987654",
                    Email = "BeyondFood@gmail.com",
                    Salary = "150$",
                    Description = "Our main objective is to leverage...",
                    Rating = 4,
                    ImageUrl = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSqBzemu65d0_r5XLDHA2yTwDMz_LZ_VdlsyAPozbIVVA&s",
                    Instagram = "https://www.instagram.com/beyondfood.lb?igsh=MXdkYWRoOGZ5Nm41Mw==",
                    Facebook = null,
                    LinkedIn = null,
                    Twitter = null,
                    LocationId = 2,
                    UserId = user1.Id
                },
                new MarketingAgency
                {
                    MarketingAgencyId = 2,
                    Username = "Explorernico",
                    PhoneNumber = "123456",
                    Email = "Explorernico@gmail.com",
                    Salary = "225$",
                    Description = "Catering to the diverse needs...",
                    Rating = 5,
                    ImageUrl = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS1314X2xOaOYisxpppju84_U0mxInsp_3rYMX5KYjs2Tuk9m9xuvL7pUS_nal3vG7YZ8A&usqp=CAU",
                    Instagram = "https://www.instagram.com/explorernico?igsh=MXd1cWRkMnN2bWtuNw==",
                    Facebook = null,
                    LinkedIn = null,
                    Twitter = null,
                    LocationId = 3,
                    UserId = user2.Id
                },
                new MarketingAgency
                {
                    MarketingAgencyId = 3,
                    Username = "Evolve",
                    PhoneNumber = "741258",
                    Email = "Evolve@gmail.com",
                    Salary = "300$",
                    Description = "Driven by a relentless pursuit...",
                    Rating = 5,
                    ImageUrl = "https://media.licdn.com/dms/image/C560BAQGRK1Kf6VCE8A/company-logo_200_200/0/1678115779285/evolvebst_logo?e=2147483647&v=beta&t=zytDG9Btn9NVSL_LZ1UL67zOASMjHvW8sGiY8nmQmOg",
                    Instagram = "https://www.instagram.com/evolve.pd?igsh=ZGV0ZDVzdTFxNm16",
                    Facebook = null,
                    LinkedIn = null,
                    Twitter = null,
                    LocationId = 4,
                    UserId = user3.Id
                },
                new MarketingAgency
                {
                    MarketingAgencyId = 4,
                    Username = "Food Couple",
                    PhoneNumber = "852366",
                    Email = "FoodCouple@gmail.com",
                    Salary = "200$",
                    Description = "We're dedicated to elevating brands...",
                    Rating = 5,
                    ImageUrl = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT0bf9-VN81pZVNblRCFeLHWXo9IlZYxheinxmXtJ9k5X06olNmg5uXgnsZMHHCkd7LScU&usqp=CAU",
                    Instagram = "https://www.instagram.com/foodcouplelb?igsh=bGdqYW5qam5ocW9n",
                    Facebook = null,
                    LinkedIn = null,
                    Twitter = null,
                    LocationId = 5,
                    UserId = user4.Id
                },
                 new MarketingAgency
                 {
                     MarketingAgencyId = 5,
                     Username = "Social Media Queen",
                     PhoneNumber = "9876543210",
                     Email = "socialmediaqueen@gmail.com",
                     Salary = "180$",
                     Description = "Expert in social media marketing and content creation. We help brands engage their audience with compelling content.",
                     Rating = 5,
                     ImageUrl = "data:image/png;base64,",
                     Instagram = "https://www.instagram.com/socialmediaqueen",
                     Facebook = "https://www.facebook.com/socialmediaqueen",
                     LinkedIn = "https://www.linkedin.com/in/socialmediaqueen",
                     Twitter = "https://twitter.com/socialqueen",
                     LocationId = 7,
                     UserId = user5.Id
                 },
    new MarketingAgency
    {
        MarketingAgencyId = 6,
        Username = "Tech Trendsetter",
        PhoneNumber = "1234567890",
        Email = "techtrendsetter@gmail.com",
        Salary = "200$",
        Description = "Stay ahead in tech with our latest reviews and gadget launches. We bring the future of technology to you.",
        Rating = 5,
        ImageUrl = "data:image/jpeg;base64,",
        Instagram = "https://www.instagram.com/techtrendsetter",
        Facebook = "https://www.facebook.com/techtrendsetter",
        LinkedIn = "https://www.linkedin.com/in/techtrendsetter",
        Twitter = "https://twitter.com/techtrendset",
        LocationId = 8,
        UserId = user6.Id
    },
    new MarketingAgency
    {
        MarketingAgencyId = 7,
        Username = "Fitness Freak",
        PhoneNumber = "1122334455",
        Email = "fitnessfreak@gmail.com",
        Salary = "150$",
        Description = "Join us on a journey to health and wellness with tailored fitness programs and healthy lifestyle tips.",
        Rating = 5,
        ImageUrl = "data:image/png;base64,",
        Instagram = "https://www.instagram.com/fitnessfreak",
        Facebook = "https://www.facebook.com/fitnessfreak",
        LinkedIn = "https://www.linkedin.com/in/fitnessfreak",
        Twitter = "https://twitter.com/fitness_freak",
        LocationId = 9,
        UserId = user7.Id
    },
    new MarketingAgency
    {
        MarketingAgencyId = 8,
        Username = "Travel Junkie",
        PhoneNumber = "9988776655",
        Email = "traveljunkie@gmail.com",
        Salary = "220$",
        Description = "Discover new destinations and travel tips from our seasoned experts who live to explore the world.",
        Rating = 4,
        ImageUrl = "https://images.unsplash.com/photo-1506748686214-e9df14d4d9d0",
        Instagram = "https://www.instagram.com/traveljunkie",
        Facebook = "https://www.facebook.com/traveljunkie",
        LinkedIn = "https://www.linkedin.com/in/traveljunkie",
        Twitter = "https://twitter.com/travel_junkie",
        LocationId = 10,
        UserId = user8.Id
    },
    new MarketingAgency
    {
        MarketingAgencyId = 9,
        Username = "Style Guru",
        PhoneNumber = "6677889900",
        Email = "styleguru@gmail.com",
        Salary = "200$",
        Description = "Your go-to source for fashion advice and trend updates, ensuring you stay stylish and on point.",
        Rating = 5,
        ImageUrl = "data:image/png;base64,",
        Instagram = "https://www.instagram.com/styleguru",
        Facebook = "https://www.facebook.com/styleguru",
        LinkedIn = "https://www.linkedin.com/in/styleguru",
        Twitter = "https://twitter.com/style_guru",
        LocationId = 11,
        UserId = user9.Id
    },
    new MarketingAgency
    {
        MarketingAgencyId = 10,
        Username = "Gaming Enthusiast",
        PhoneNumber = "5566778899",
        Email = "gamingenthusiast@gmail.com",
        Salary = "250$",
        Description = "Level up with the latest gaming news, reviews, and live streams. Your ultimate gaming destination.",
        Rating = 5,
        ImageUrl = "https://images.unsplash.com/photo-1593642632823-8f785ba67e45",
        Instagram = "https://www.instagram.com/gamingenthusiast",
        Facebook = "https://www.facebook.com/gamingenthusiast",
        LinkedIn = "https://www.linkedin.com/in/gamingenthusiast",
        Twitter = "https://twitter.com/gaming_enthusiast",
        LocationId = 12,
        UserId = user10.Id
    }
            );

            
            modelBuilder.Entity<Post>().HasData(
    new Post
    {
        PostId = 1,
        Caption = "BeyondFood Post 1",
        Content = "https://www.youtube.com/watch?v=dQw4w9WgXcQ",
        picContent = "https://images.unsplash.com/photo-1512621776951-a57141f2eefd",
        MarketingAgencyId = 1
    },
    
    new Post
    {
        PostId = 4,
        Caption = "BeyondFood Post 4",
        Content = "https://www.youtube.com/watch?v=ZZ5LpwO-An4",
        picContent = "https://images.unsplash.com/photo-1504674900247-0877df9cc836",
        MarketingAgencyId = 1
    },
    
    new Post
    {
        PostId = 6,
        Caption = "Explorernico Post 1",
        Content = "https://www.youtube.com/watch?v=ZXsQAXx_ao0",
        picContent = "https://images.unsplash.com/photo-1506748686214-e9df14d4d9d0",
        MarketingAgencyId = 2
    },
    new Post
    {
        PostId = 7,
        Caption = "Explorernico Post 2",
        Content = "https://www.youtube.com/watch?v=2vjPBrBU-TM",
        picContent = "https://images.unsplash.com/photo-1556740749-887f6717d7e4",
        MarketingAgencyId = 2
    },
    
    new Post
    {
        PostId = 11,
        Caption = "Evolve Post 1",
        Content = "https://www.youtube.com/watch?v=aqz-KE-bpKQ",
        picContent = "https://images.unsplash.com/photo-1529156069898-49953e39b3ac",
        MarketingAgencyId = 3
    },
   
    new Post
    {
        PostId = 14,
        Caption = "Evolve Post 4",
        Content = "https://www.youtube.com/watch?v=8UVNT4wvIGY",
        picContent = "https://images.unsplash.com/photo-1593642532973-d31b6557fa68",
        MarketingAgencyId = 3
    },
    new Post
    {
        PostId = 15,
        Caption = "Evolve Post 5",
        Content = "https://www.youtube.com/watch?v=J---aiyznGQ",
        picContent = "https://images.unsplash.com/photo-1560807707-8cc77767d783",
        MarketingAgencyId = 3
    },
    
    new Post
    {
        PostId = 19,
        Caption = "Food Couple Post 4",
        Content = "https://www.youtube.com/watch?v=ktvTqknDobU",
        picContent = "https://images.unsplash.com/photo-1506089676908-3592f7389d4d",
        MarketingAgencyId = 4
    },
    new Post
    {
        PostId = 20,
        Caption = "Food Couple Post 5",
        Content = "https://www.youtube.com/watch?v=9bZkp7q19f0",
        picContent = "https://images.unsplash.com/photo-1495567720989-cebdbdd97913",
        MarketingAgencyId = 4
    },
    new Post
    {
        PostId = 21,
        Caption = "Social Media Queen Post 1",
        Content = "https://www.youtube.com/watch?v=CevxZvSJLk8",
        picContent = "https://images.unsplash.com/photo-1536243405184-d91150a7b25f",
        MarketingAgencyId = 5
    },
    new Post
    {
        PostId = 22,
        Caption = "Social Media Queen Post 2",
        Content = "https://www.youtube.com/watch?v=3AtDnEC4zak",
        picContent = "https://images.unsplash.com/photo-1500522144261-ea64433bbe27",
        MarketingAgencyId = 5
    },
    
    new Post
    {
        PostId = 26,
        Caption = "Tech Trendsetter Post 1",
        Content = "https://www.youtube.com/watch?v=opVNlmjOeSE",
        picContent = "https://images.unsplash.com/photo-1560807707-8cc77767d783",
        MarketingAgencyId = 6
    },
    
    new Post
    {
        PostId = 29,
        Caption = "Tech Trendsetter Post 4",
        Content = "https://www.youtube.com/watch?v=3JZ_D3ELwOQ",
        picContent = "https://images.unsplash.com/photo-1593642532973-d31b6557fa68",
        MarketingAgencyId = 6
    },
    new Post
    {
        PostId = 30,
        Caption = "Tech Trendsetter Post 5",
        Content = "https://www.youtube.com/watch?v=Zi_XLOBDo_Y",
        picContent = "https://images.unsplash.com/photo-1560807707-8cc77767d783",
        MarketingAgencyId = 6
    },
    
    new Post
    {
        PostId = 34,
        Caption = "Fitness Freak Post 4",
        Content = "https://www.youtube.com/watch?v=ktvTqknDobU",
        picContent = "https://images.unsplash.com/photo-1506089676908-3592f7389d4d",
        MarketingAgencyId = 7
    },
    new Post
    {
        PostId = 35,
        Caption = "Fitness Freak Post 5",
        Content = "https://www.youtube.com/watch?v=ZZ5LpwO-An4",
        picContent = "https://images.unsplash.com/photo-1495567720989-cebdbdd97913",
        MarketingAgencyId = 7
    },
    new Post
    {
        PostId = 36,
        Caption = "Travel Junkie Post 1",
        Content = "https://www.youtube.com/watch?v=dQw4w9WgXcQ",
        picContent = "https://images.unsplash.com/photo-1512621776951-a57141f2eefd",
        MarketingAgencyId = 8
    },
   
    new Post
    {
        PostId = 39,
        Caption = "Travel Junkie Post 4",
        Content = "https://www.youtube.com/watch?v=RgKAFK5djSk",
        picContent = "https://images.unsplash.com/photo-1504674900247-0877df9cc836",
        MarketingAgencyId = 8
    },
    
    new Post
    {
        PostId = 41,
        Caption = "Style Guru Post 1",
        Content = "https://www.youtube.com/watch?v=nfWlot6h_JM",
        picContent = "https://images.unsplash.com/photo-1506748686214-e9df14d4d9d0",
        MarketingAgencyId = 9
    },
    new Post
    {
        PostId = 42,
        Caption = "Style Guru Post 2",
        Content = "https://www.youtube.com/watch?v=2vjPBrBU-TM",
        picContent = "https://images.unsplash.com/photo-1556740749-887f6717d7e4",
        MarketingAgencyId = 9
    },
   
    new Post
    {
        PostId = 46,
        Caption = "Gaming Enthusiast Post 1",
        Content = "https://www.youtube.com/watch?v=aqz-KE-bpKQ",
        picContent = "https://images.unsplash.com/photo-1529156069898-49953e39b3ac",
        MarketingAgencyId = 10
    },
   
    new Post
    {
        PostId = 49,
        Caption = "Gaming Enthusiast Post 4",
        Content = "https://www.youtube.com/watch?v=8UVNT4wvIGY",
        picContent = "https://images.unsplash.com/photo-1593642532973-d31b6557fa68",
        MarketingAgencyId = 10
    },
    new Post
    {
        PostId = 50,
        Caption = "Gaming Enthusiast Post 5",
        Content = "https://www.youtube.com/watch?v=J---aiyznGQ",
        picContent = "https://images.unsplash.com/photo-1560807707-8cc77767d783",
        MarketingAgencyId = 10
    }
);
        }

        internal string? FindAsync(string? customerId)
        {
            throw new NotImplementedException();
        }


    }
}